      ...
      MaterialPageRoute(
        builder: (context) => WorkoutInProgressPage(
          exercises: _exercises
              .map(
                (e) => _WorkoutExerciseDraft(e.exerciseName, [
                  ...e.sets.map(
                    (s) => _WorkoutSetDraft(reps: s.reps, weight: s.weight),
                  ),
                ]),
              )
              .toList(),
...
class _WorkoutExerciseDraft {
  ...
  String get setsSummary =>
      sets.map((s) => '${s.reps} x ${s.weight} lbs').join(', ');
}
...
class _WorkoutInProgressPageState extends State<WorkoutInProgressPage> {
  ...
  Future<void> _showAddExerciseModal() async {
    ...
    if (selectedExercise != null) {
      setState(() {
        final newExercise = _WorkoutExerciseDraft(selectedExercise.name, [
