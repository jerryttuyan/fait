...
import 'package:fait/data/StatsData/weight_entry.dart'; // Import the Isar model
...
class _WeightTrackingPageState extends State<WeightTrackingPage> {
  ...
  Widget build(BuildContext context) {
    final units = Provider.of<AppSettings>(context).units;
    String label = units == Units.metric ? 'Enter current weight (kg)' : 'Enter current weight (lb)';
    return Padding(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (weight != null && units == Units.metric) {
                weight = weight * 2.20462; // kg to lb
              }
              if (weight != null) {
                final newEntry = WeightEntry()
                  ..weight = weight
